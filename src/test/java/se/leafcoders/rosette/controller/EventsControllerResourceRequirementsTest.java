...
public class EventsControllerResourceRequirementsTest extends AbstractControllerTest {
    ...
    private Matcher<?> RESOURCE_REQUIREMENT_PARAMS = allOf(hasKey("id"), hasKey("resourceType"), hasKey("resources"));
    ...
    public void addResourceRequirementToEvent() throws Exception {
        ...
        crt.postSuccessWithOk(user1, "/events/" + event.getId() + "/resourceRequirements", jsonData2)
            .andExpect(jsonPath("$", hasSize(2)))
            .andExpect(jsonPath("$[0]", RESOURCE_REQUIREMENT_PARAMS))
            .andExpect(jsonPath("$[1]", RESOURCE_REQUIREMENT_PARAMS));

        // Add the same resource type shall fail
        crt.postExpectForbidden(ApiError.CHILD_ALREADY_EXIST, user1, "/events/" + event.getId() + "/resourceRequirements", jsonData2);
    }
    ...
    public void getResourceRequirementsOfEvent() throws Exception {
        user1 = givenUser(user1);
        Event event = eventRepository.save(EventData.existingEvent(eventType));
        ResourceRequirement resourceRequirement1 = resourceRequirementRepository.save(ResourceRequirementData.create(event, resourceType1));
        ResourceRequirement resourceRequirement2 = resourceRequirementRepository.save(ResourceRequirementData.create(event, resourceType2));
        event.addResourceRequirement(resourceRequirement1);
        event.addResourceRequirement(resourceRequirement2);
        event = eventRepository.save(event);

        crt.allGetChildrenTests(user1, "resourceTypes:read,events:read", "/events/" + event.getId() + "/resourceRequirements")
            .andExpect(jsonPath("$", hasSize(2)))
            .andExpect(jsonPath("$[0]", RESOURCE_REQUIREMENT_PARAMS))
            .andExpect(jsonPath("$[1]", RESOURCE_REQUIREMENT_PARAMS));
    }
    ...
    public void removeResourceRequirementFromEvent() throws Exception {
        ...
        crt.allRemoveChildTests(user1, "resourceTypes:read,events:read,events:update", "/events/" + event.getId() + "/resourceRequirements", resourceRequirement2.getId())
            .andExpect(jsonPath("$", hasSize(1)))
            .andExpect(jsonPath("$[0]", RESOURCE_REQUIREMENT_PARAMS))
